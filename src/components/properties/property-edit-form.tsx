...
export function PropertyEditForm({ property }: PropertyEditFormProps) {
  ...
  const [lotSize, setLotSize] = useState(() => str(property.lot_size_sqft));
  const [built, setBuilt] = useState(() => str(property.built_floor_area_sqft));
  const [maxFar, setMaxFar] = useState(() => str(property.max_far));
  const [ratePerSf, setRatePerSf] = useState(() =>
    property.estimated_value_per_sqft == null
      ? ""
      : str(property.estimated_value_per_sqft),
  );
  const [constructionPerSf, setConstructionPerSf] = useState(() =>
    property.construction_cost_per_sqft == null
      ? ""
      : str(property.construction_cost_per_sqft),
  );
  const [softPct, setSoftPct] = useState(() =>
    String(property.soft_cost_percentage ?? DEFAULT_SOFT_COST_PCT),
  );
  const [exitPerSf, setExitPerSf] = useState(() =>
    property.exit_value_per_sqft == null ? "" : str(property.exit_value_per_sqft),
  );
  ...
  const live = useMemo(() => {
    ...
    if (!Number.isFinite(maxF) || maxF <= 0) return null;
    if (!Number.isFinite(builtN) || builtN < 0) return null;
    ...
    <form action={formAction} className="mx-auto max-w-2xl space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <select
            id="status"
            name="status"
            defaultValue={normalizePropertyStatus(property.status)}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2 sm:col-span-2">
            ...
            <Input
              ...
              required
              defaultValue={property.zoning_district}
            />
      ...
      <section className="rounded-lg border border-neutral-200 bg-neutral-50/80 p-5">
        ...
        <p className="mt-1 text-xs text-neutral-500">
          Updates as you type. Derived fields are recomputed on save.
        </p>
        {live ? (
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-neutral-500">
              Default {DEFAULT_SOFT_COST_PCT}% of hard construction.
            </p>
      ...
      <section className="rounded-lg border border-neutral-200 bg-neutral-50/80 p-5">
        ...
        <p className="mt-1 text-xs text-neutral-500">
          Exit value falls back to est. value per buildable sq ft when omitted.
        </p>
        {liveDev ? (
          <dl className="mt-4 grid gap-3 sm:grid-cols-2">
            <EditMetric
